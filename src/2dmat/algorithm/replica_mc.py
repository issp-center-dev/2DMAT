...
class Algorithm(algorithm.Algorithm):
    ...
    #TODO: make parallel montecalro algorithm
...
class Init_Param(surf_base.Init_Param):
    ...
    def from_dict(cls, dict):
        ...
        info["param"]["nreplicas"] = dict["nreplicas"]
        info["param"]["nprocs_per_replica"] = dict["nprocs_per_replica"]
        info["param"]["kTstart"] = dict["kTstart"]
        info["param"]["kTend"] = dict["kTend"]
        info["param"]["nsteps"] = dict["nsteps"]
        info["param"]["RXtrial_frequency"] = dict.get("RXtrial_frequency", 1)
        info["param"]["sample_frequency"] = dict.get("sample_frequency", 1)
        info["param"]["print_frequency"] = dict.get("print_frequency", 1)
        info["param"]["reload"] = dict.get("reload", False)
        info["param"]["seed"] = dict.get("seed", 0)
        return info
...
def MPI_Init(info):
    ...
    nreplicas = info["param"]["nreplicas"]
    commworld = MPI.COMM_WORLD
    worldrank = commworld.Get_rank()
    worldprocs = commworld.Get_size()
    if info["param"]["seed"] > 0:
        rand.seed(info["param"]["seed"] + worldrank * 137)
    else:
    ...
    if worldprocs > nreplicas:
        ...
        comm = commworld
    info["mpi"]["comm"] = comm
    info["mpi"]["size"] = worldprocs
    info["mpi"]["rank"] = worldrank
    return info